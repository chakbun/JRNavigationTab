...
- (void)tapActionWithViewTag:(NSInteger)viewTag {
    ...
    if (self.tabSelectedBlock) {
        _seletedIndex = viewTag - TAG_ITEM_HEAD;
...
- (void)setTabText:(NSString *)text tabIndex:(NSInteger)index {
    JRNavTabItem *item = [self viewWithTag:TAG_ITEM_HEAD + index];
    item.title4Normal = text;
}
...
- (void)setTabTextColor:(UIColor *)color hightLightColor:(UIColor *)hlColor {
    for(UIView *view in [self subviews]) {
        if([view isKindOfClass:[JRNavTabItem class]]) {
            JRNavTabItem *item = (JRNavTabItem *)view;
            item.color4Title4Normal = color;
            item.color4Title4Highlight = hlColor;
        }
    }
}
...
- (void)setSeletedIndex:(NSInteger)seletedIndex {
    ...
    [self tapActionWithViewTag:seletedIndex + TAG_ITEM_HEAD];
